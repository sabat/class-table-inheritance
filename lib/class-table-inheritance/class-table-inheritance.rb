  ...
  class Base  
    ...
    class << self
      ...
      def inherited_column_names
        ...
        inherited_column_names.reject { |c| self.column_names.grep(c).length > 0 || %w{ type subtype id }.include?(c) }
      end
      ...
      def inherits_from(assoc_id)
        # Subst the module symbol to dash if this is string
    ...
    def inspect
      if inherits?
        inspection =
            if attributes
              column_names = self.class.column_names + self.class.inherited_column_names
